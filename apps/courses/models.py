...
def convert_to_embed_url(url: str) -> str:
    """
    Converts YouTube (Standard, Unlisted, Live Streams, Shorts, Embeds) and Vimeo links
    into a clean, secure, responsive embed URL with zero-friction iframe compatibility.
    """
    if not url:
        return ""
    url = url.strip()
...
class Course(TenantAwareModel):
    ...
    @property
    def embed_intro_video_url(self) -> str:
        """Returns iframe-ready YouTube / Live embed URL."""
        return convert_to_embed_url(self.intro_video_url)
...
class Lesson(TenantAwareModel):
    ...
    @property
    def embed_video_url(self) -> str:
        """Returns iframe-ready YouTube / Live embed URL."""
        return convert_to_embed_url(self.video_url)
...
class FeedPost(TenantAwareModel):
    ...
    class Category(models.TextChoices):
        ANNOUNCEMENT = "announcement", "📢 নোটিশ / ঘোষণা"
